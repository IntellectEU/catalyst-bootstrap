  ...
  applyParams = function () {
    ...
    $.each(params, function (index, param) {
      ...
      switch (param.name) {
        ...
        case 'javaVersion':
        case 'language':
          $('.' + param.name.toLowerCase() + '-form-group').removeClass(
              "hidden");
          $('#' + param.name + ' option[value="' + value + '"]').prop(
              'selected', true);
          $('#' + param.name).change();
          break;
        case 'groupId':
...
$(function () {
  ...
  var refreshDependencies = function (versionRange) {
    ...
    $("#dependencies div.checkbox").each(function (idx, item) {
      if (!$(item).attr('data-range') || versions.matchRange(
              $(item).attr('data-range'))(versionRange)) {
  ...
  $("#type").on('change', function () {
    $("#form").attr('action',
        $(this.options[this.selectedIndex]).attr('data-action'))
  });
  ...
  $('#autocomplete').typeahead(
      {
        ...
        templates: {
          suggestion: function (data) {
            return "<div><strong>" + data.name + "</strong><br/><small>"
                + data.description + "</small></div>";
          },
          footer: function (search) {
            if (search.suggestions && search.suggestions.length
                == maxSuggestions) {
