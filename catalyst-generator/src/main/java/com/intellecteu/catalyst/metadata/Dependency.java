...
/**
 * Meta-data for a dependency. Each dependency has a primary identifier and an arbitrary number of
 * {@code aliases}.
 *
...
public class Dependency extends MetadataElement implements Describable {
  ...
  /**
   * Specify if the dependency represents a "starter", i.e. the sole presence of that dependency is
   * enough to bootstrap the context.
   */
  ...
  /**
   * Define this dependency as a standard spring boot starter with the specified name <p> If no name
   * is specified, the root "spring-boot-starter" is assumed.
   */
  ...
  /**
   * Resolve this instance according to the specified Spring Boot {@link Version}. Return a {@link
   * Dependency} instance that has its state resolved against the specified version.
   */
  ...
  public boolean match(Version version) {
    return range == null || range.match(version);
  }
  ...
  /**
   * Return the default version, can be {@code null} to indicate that the version is managed by the
   * project and does not need to be specified.
   */
  ...
  /**
   * Return the type, can be {@code null} to indicate that the default type should be used (i.e.
   * {@code jar}).
   */
  ...
  /**
   * Return the dependency mapping if an attribute of the dependency differs according to the Spring
   * Boot version. If no mapping matches, default attributes are used.
   */
