...
/**
 * Define a Bill Of Materials to be represented in the generated project if a dependency refers to
 * it.
 *
...
public class BillOfMaterials {
  ...
  /**
   * Return the {@link VersionProperty} to use to externalize the version of the BOM. When this is
   * set, a version property is automatically added rather than setting the version in the bom
   * declaration itself.
   */
  ...
  /**
   * Return the relative order of this BOM where lower values have higher priority. The default
   * value is {@code Integer.MAX_VALUE}, indicating lowest priority. The Spring Boot dependencies
   * bom has an order of 100.
   */
  ...
  /**
   * Return the BOM(s) that should be automatically included if this BOM is required. Can be {@code
   * null} if it is provided via a mapping.
   */
  ...
  /**
   * Return the repositories that are required if this BOM is required. Can be {@code null} if it is
   * provided via a mapping.
   */
   ...
   * BillOfMaterials} instance that holds the version, repositories and additional BOMs to use, if
   * any.
   */
